...
class MyKeyboardListener(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.redrawcanvas(isnewgame=True)

    def start(self,_):

        if self.isPlaying == False and self.hasLost:
            self.interval = 0.45
            self.shouldMove = 'right'
            self.currentDirection = 'horizontal'
            self.gridItemsOccupiedBySnake = [[100,100]]
            self.score = 0
            self.hasLost = False
            self.redrawcanvas(isnewgame=True)
    ...
    def setLost(self):
        ...
        self.hasLost = True
        self.redrawcanvas(haslost=True)
    ...
    def updatesnake(self,newx,newy):
        ...
        self.redrawcanvas(isupdate=True, shoulddrawnewfood=hasCollidedWithFood)
    ...
    def updateSnakePosition(self,newx,newy,shouldIncreaseLength):
    
        updatedGridItemsOccupiedBySnake = [[newx,newy]]
        positionstobemerged = []

        if shouldIncreaseLength:
            positionstobemerged = self.gridItemsOccupiedBySnake
        else:
            self.gridItemsOccupiedBySnake.pop()
            positionstobemerged = self.gridItemsOccupiedBySnake

        for x in positionstobemerged:
            updatedGridItemsOccupiedBySnake.append(x)
        
        self.gridItemsOccupiedBySnake = updatedGridItemsOccupiedBySnake
    ...
    def redrawcanvas(self, isnewgame=False, isupdate=False, haslost=False, shoulddrawnewfood=False):
        self.canvas.clear()
        ...
        score = Button(text='score: '+str(self.score),
                            size=(200,50),
                            pos=(1100,1050))
        ...
        with self.canvas:
            ...
            self.snake = []
            for snakeelement in self.gridItemsOccupiedBySnake:
                self.snake.append(Rectangle(pos=(snakeelement[0],snakeelement[1]),size=(40,40)))
